...
import ProductPriceDisplay from '../pricing/prodDetailPricer';
...
interface HeroProps {
    ...
    ProductOftheMonth?: Product;
}

export default function Hero({ featuredProducts, ProductOftheMonth }: HeroProps) {
                ...
                <div className="grid lg:grid-cols-[1.2fr_1fr] gap-12 items-center">
                    ...
                    <motion.div initial={{ opacity: 0, x: -20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6 }} className="space-y-6">
                        ...
                        >
                            ...
                            Indepenedence Sale Live - 50-70% Off
                        </div>
                        ...
                        >
                            {Object.values(featuredProducts).map((product, index) => (
                    ...
                    >
                        <div className="space-y-4 text-center">
                            ...
                            <h2 className="text-3xl font-bold dark:text-white">{ProductOftheMonth?.name}</h2>
                            <p className="flex text-primary-500 text-2xl font-bold !justify-center !items-center text-center ">{ProductOftheMonth && <ProductPriceDisplay productDetail={ProductOftheMonth} />}</p>
                            ...
                            <Link to={`/products/${ProductOftheMonth.id}`}
                                className="btn w-full py-4 bg-primary-500 hover:bg-priamry-600 text-white rounded-xl font-medium transition-colors duration-200 transform hover:scale-[1.02] active:scale-95"
    ...
    return (
        ...
        >
            <div className="relative mb-3 h-40">
                <img src={product.main_product_image} alt="" className="object-contain" loading="lazy" aria-hidden="true" />
            ...
            <div className="flex items-center justify-between mt-2">
                <span className="text-primary-500 font-bold">{formatPrice(product.price)}</span>
                <Link
                    to={`/products/${product.id}`}
                    className="text-sm bg-gray-100 dark:bg-gray-700 px-3 py-1.5 rounded-md transition-colors hover:bg-gray-200 dark:hover:bg-gray-600"
